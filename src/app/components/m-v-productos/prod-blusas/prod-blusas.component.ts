...
@Component({
  selector: 'app-prod-blusas',
  templateUrl: './prod-blusas.component.html',
  styleUrls: ['./prod-blusas.component.css']
})
export class ProdBlusasComponent {
  ...
  getOnlyBlusas() {
    ...
    {
      this.listProducts = data.filter(op=>op.idcat == 3);
    });
  }
    ...
    SizeColorDetector(){
      ...
      if(this.colorSeleccionada != null && this.tallaSeleccionada != null){
        ...
        this._productService.getProducts().subscribe(
          (options: any[]) => {
            this.listProducts = options.filter(option=>option.idcolor == this.colorSeleccionada && option.idcat == 3);
        ...
        this._productService.getProducts().subscribe(
          (options: any[]) => {
            this.listProducts = options.filter(option=>option.idsize == this.tallaSeleccionada && option.idcat == 3);
            console.log("LOG1",this.listProducts)
          },
          (error: any) => {
            console.log(error);
  ...
  handleComboChange(size: any, color: any){
        if(color!= null && size != null){
          ...
          this._productService.getProducts().subscribe(
            (options: any[]) => {
              this.listProducts = options.filter(option=>option.idsize == size && option.idcolor == color && option.idcat == 3);
              console.log("LOG1",this.listProducts)
            },
            (error: any) => {
