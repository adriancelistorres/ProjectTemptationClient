...
export class ProdBlusasComponent {
  ...
  selectSize_S(){
    this._productService.getProducts().subscribe(
      (options: any[]) => {
        this.listProducts = options.filter(option=>option.idsize == 1 && option.idcat == 3);
  ...
  selectSize_M(){
    this._productService.getProducts().subscribe(
      (options: any[]) => {
        this.listProducts = options.filter(option=>option.idsize == 2 && option.idcat == 3);
        console.log("LOG1",this.listProducts)
      },
      (error: any) => {
        console.log(error);
  ...
  selectSize_L(){
    this._productService.getProducts().subscribe(
      (options: any[]) => {
        this.listProducts = options.filter(option=>option.idsize == 3 && option.idcat == 3);
        console.log("LOG1",this.listProducts)
      },
      (error: any) => {
        console.log(error);
