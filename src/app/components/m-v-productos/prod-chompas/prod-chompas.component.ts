...
export class ProdChompasComponent {
    ...
    SizeColorDetector(){
      ...
      if(this.colorSeleccionada != null && this.tallaSeleccionada != null){
        ...
        this._productService.getProducts().subscribe(
          (options: any[]) => {
            this.listProducts = options.filter(option=>option.idcolor == this.colorSeleccionada && option.idcat == 4);
        ...
        this._productService.getProducts().subscribe(
          (options: any[]) => {
            this.listProducts = options.filter(option=>option.idsize == this.tallaSeleccionada && option.idcat == 4);
            console.log("LOG1",this.listProducts)
          },
          (error: any) => {
            console.log(error);
  ...
  handleComboChange(size: any, color: any){
        if(color!= null && size != null){
          ...
          this._productService.getProducts().subscribe(
            (options: any[]) => {
              this.listProducts = options.filter(option=>option.idsize == size && option.idcolor == color && option.idcat == 4);
              console.log("LOG1",this.listProducts)
            },
            (error: any) => {
